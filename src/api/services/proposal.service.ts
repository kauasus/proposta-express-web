...
const delay = async (): Promise<void> => new Promise((resolve) => setTimeout(resolve, 450))
...
export const proposalService = {
  ...
  async create(payload: ProposalInput): Promise<Proposal> {
    ...
    const proposal: Proposal = payload.notes ? { ...base, notes: payload.notes } : base
  ...
  async send(id: string): Promise<Proposal> {
    ...
    const updated: Proposal = { ...current, status: 'sent', updatedAt: nowIso() }
    proposals[index] = updated
    mockDb.saveProposals(proposals)
    return updated
  },
  ...
  async getPublicByToken(publicToken: string): Promise<Proposal> {
    await delay()
    const proposal = mockDb.getProposals().find((item) => item.publicToken === publicToken)
    if (!proposal) throw new Error('Proposta não encontrada')
    return proposal
  },
  ...
  async trackView(publicToken: string): Promise<void> {
    await delay()
    const proposals = mockDb.getProposals()
    const index = proposals.findIndex((item) => item.publicToken === publicToken)
    if (index < 0) return
  ...
  async accept(publicToken: string): Promise<Proposal> {
    ...
    const updated: Proposal = { ...current, status: 'accepted', updatedAt: nowIso() }
    proposals[index] = updated
    mockDb.saveProposals(proposals)
    return updated
  },
